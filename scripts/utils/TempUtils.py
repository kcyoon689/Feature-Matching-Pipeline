...
class TempUtils:
    ...
    def make_concat_rotated_images(
        ...
        h, w, c = img1_resized.shape
        hmargin = np.ones((h, max(1, int(w / 100)), c), dtype=np.float32)
        img1_concat_rotated = cv2.hconcat(
            [img1_resized_rgb_float, hmargin, img1_resized_r_tf_rgb_float]
        )
        return img1_concat_rotated

    def concat_images_different_size(img0: np.ndarray, img1: np.ndarray) -> np.ndarray:
        h0, w0, c0 = img0.shape
        h1, w1, c1 = img1.shape
        hmargin = np.ones((max(h0, h1), max(1, int(w1 / 100)), c1), dtype=np.float32)
        canvas = np.zeros(
            (max(h0, h1), w0 + max(1, int(w1 / 100)) + w1, c0), dtype=np.float32
        )
        canvas[max(0, h1 - h0) : max(0, h1 - h0) + h0, 0:w0] = np.copy(img0)
        canvas[0 : max(h0, h1), w0 : w0 + max(1, int(w1 / 100))] = np.copy(hmargin)
        canvas[
    ...
    def apply_transformation_to_image_feature(
        image_feature, transformation, axis_order
    ):
        ...
        image_feature[axis_order[0] + "_r"] = (
            transformation[0][0] * image_feature[axis_order[0]]
            + transformation[0][1] * image_feature[axis_order[1]]
        )
        image_feature[axis_order[1] + "_r"] = (
            transformation[1][0] * image_feature[axis_order[0]]
            + transformation[1][1] * image_feature[axis_order[1]]
        )
        image_feature[axis_order[0] + "_tf"] = (
            image_feature[axis_order[0] + "_r"] + transformation[0][2]
        )
        image_feature[axis_order[1] + "_tf"] = (
            image_feature[axis_order[1] + "_r"] + transformation[1][2]
        )
    ...
    def apply_offset_to_image_feature(image_feature, plane_offset, axis_order, idx):
        ...
        image_feature[axis_order[0] + "_tf"] += plane_offset[axis_order[0]][idx]
        image_feature[axis_order[1] + "_tf"] += plane_offset[axis_order[1]][idx]

        return image_feature

    # remove unused argument

    def create_xy_rgb_surface(
        cv_img_rgb, z, x_offset=0, y_offset=0, black_criteria=50, **kwargs
    ):
        ...
        x, y = np.linspace(
            x_offset, x_offset + eight_bit_img.width, eight_bit_img.width
        ), np.linspace(y_offset, y_offset + eight_bit_img.height, eight_bit_img.height)
    ...
    def create_yz_rgb_surface(
        cv_img_rgb, x, y_offset=0, z_offset=0, black_criteria=50, **kwargs
    ):
        cv_img_rgb = (cv_img_rgb * 255).astype(np.uint8)
        cv_img_rgb = cv_img_rgb.swapaxes(0, 1)[:, ::-1]
        eight_bit_img = PImage.fromarray(cv_img_rgb, mode="RGB").convert(
        ...
        z, y = np.linspace(
            z_offset, z_offset + eight_bit_img.width, eight_bit_img.width
        ), np.linspace(y_offset, y_offset + eight_bit_img.height, eight_bit_img.height)
        idx_to_color = np.array(eight_bit_img.getpalette()).reshape((-1, 3))
        colorscale = [
            [i / 255.0, "rgb({}, {}, {})".format(*rgb)]
    ...
    def get_plot_data_image(
        cv_image_list, plane_offset={"x": [], "y": [], "z": []}, align="h"
    ):
        plot_data_list = []

        for cv_image, x_offset, y_offset, z_offset in zip(
            cv_image_list, plane_offset["x"], plane_offset["y"], plane_offset["z"]
        ):
            if align == "h":
                plot_data_list.append(
                    create_xy_rgb_surface(
                        ...
                        y_offset=y_offset,
                        contours_z=dict(
                            show=True, project_z=True, highlightcolor="limegreen"
                        ),
                        opacity=1.0,
                    )
                )
                ...
                plot_data_list.append(
                    create_yz_rgb_surface(
                        ...
                        z_offset=z_offset,
                        contours_x=dict(
                            show=True, project_x=True, highlightcolor="limegreen"
                        ),
                        opacity=1.0,
                    )
                )
    ...
    def get_plot_data_feature_points(
        feature_df_list,
        feature_color_list,
        feature_point_size,
        axis_order,
        ...
        for idx in range(0, int(len(feature_df_list) / 2)):
            ...
            if is_plot_line:
                for line_x1, line_x2, line_y1, line_y2, line_z1, line_z2 in zip(
                    feature_df_list[2 * idx][axis_x],
                    feature_df_list[2 * idx + 1][axis_x],
                    feature_df_list[2 * idx][axis_y],
                    feature_df_list[2 * idx + 1][axis_y],
                    feature_df_list[2 * idx][axis_z],
                    feature_df_list[2 * idx + 1][axis_z],
                ):
                    plot_data_list.append(
                        go.Scatter3d(
                            x=[line_x1, line_x2],
    ...
    def get_plot_data_tf_verification(
        feature_df0, feature_df1, feature_color_list, axis_order, index
    ):
        plot_data_list = []
        ...
        plot_data_list.append(
            go.Scatter3d(
                x=feature_df0[axis_order[0] + "_tf"],
        ...
        plot_data_list.append(
            go.Scatter3d(
                x=feature_df1[axis_order[0] + "_tf"],
        ...
        for line_x0, line_x0_r, line_y0, line_y0_r, line_z0 in zip(
            feature_df0[axis_order[0]],
            feature_df0[axis_order[0] + "_r"],
            feature_df0[axis_order[1]],
            feature_df0[axis_order[1] + "_r"],
            feature_df0[axis_order[2]],
        ):
            plot_data_list.append(
                go.Scatter3d(
                    x=[line_x0, line_x0_r],
        ...
        for line_x0_r, line_x0_tf, line_y0_r, line_y0_tf, line_z0 in zip(
            feature_df0[axis_order[0] + "_r"],
            feature_df0[axis_order[0] + "_tf"],
            feature_df0[axis_order[1] + "_r"],
            feature_df0[axis_order[1] + "_tf"],
            feature_df0[axis_order[2]],
        ):
            plot_data_list.append(
                go.Scatter3d(
                    x=[line_x0_r, line_x0_tf],
        ...
        for line_x0_tf, line_x1_tf, line_y0_tf, line_y1_tf, line_z0 in zip(
            feature_df0[axis_order[0] + "_tf"],
            feature_df1[axis_order[0] + "_tf"],
            feature_df0[axis_order[1] + "_tf"],
            feature_df1[axis_order[1] + "_tf"],
            feature_df0[axis_order[2]],
        ):
            plot_data_list.append(
                go.Scatter3d(
                    x=[line_x0_tf, line_x1_tf],
        ...
        plot_data_list.append(
            go.Scatter3d(
                x=[feature_df0_mean[axis_order[0]]],
        ...
        plot_data_list.append(
            go.Scatter3d(
                x=[feature_df0_mean[axis_order[0] + "_r"]],
        ...
        plot_data_list.append(
            go.Scatter3d(
                x=[feature_df0_mean[axis_order[0] + "_tf"]],
        ...
        plot_data_list.append(
            go.Scatter3d(
                x=[feature_df1_mean[axis_order[0] + "_tf"]],
        ...
        plot_data_list.append(
            go.Scatter3d(
                x=[
                    feature_df0_mean[axis_order[0]],
                    feature_df0_mean[axis_order[0] + "_r"],
                ],
                y=[
                    feature_df0_mean[axis_order[1]],
                    feature_df0_mean[axis_order[1] + "_r"],
                ],
                z=[feature_df0_mean[axis_order[2]], feature_df0_mean[axis_order[2]]],
                mode="lines",
                marker=dict(
                    color=feature_color_list[index],
                ),
        ...
        plot_data_list.append(
            go.Scatter3d(
                x=[
                    feature_df0_mean[axis_order[0] + "_r"],
                    feature_df0_mean[axis_order[0] + "_tf"],
                ],
                y=[
                    feature_df0_mean[axis_order[1] + "_r"],
